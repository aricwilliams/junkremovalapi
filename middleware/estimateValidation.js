...
const estimateSchema = Joi.object({
  ...
  ok_to_text: Joi.boolean().optional().messages({
  ...
  gate_code: Joi.string().max(100).optional().allow('').messages({
    'string.max': 'Gate code cannot exceed 100 characters'
  }),
  apartment_unit: Joi.string().max(50).optional().allow('').messages({
  ...
  preferred_time: Joi.string().max(50).optional().allow('').messages({
  ...
  items_filled_oil_fuel: Joi.boolean().optional().messages({
    'boolean.base': 'Items filled oil/fuel must be a boolean value'
  }),
  hazardous_materials: Joi.boolean().optional().messages({
    'boolean.base': 'Hazardous materials must be a boolean value'
  }),
  items_tied_bags: Joi.boolean().optional().messages({
    'boolean.base': 'Items tied bags must be a boolean value'
  }),
  oversized_items: Joi.boolean().optional().messages({
    'boolean.base': 'Oversized items must be a boolean value'
  }),
  
  // Safety & Hazards
  mold_present: Joi.boolean().optional().messages({
    'boolean.base': 'Mold present must be a boolean value'
  }),
  pests_present: Joi.boolean().optional().messages({
    'boolean.base': 'Pests present must be a boolean value'
  }),
  sharp_objects: Joi.boolean().optional().messages({
    'boolean.base': 'Sharp objects must be a boolean value'
  }),
  heavy_lifting_required: Joi.boolean().optional().messages({
    'boolean.base': 'Heavy lifting required must be a boolean value'
  }),
  disassembly_required: Joi.boolean().optional().messages({
    'boolean.base': 'Disassembly required must be a boolean value'
  }),
  
  // Additional Information & Services
  additional_notes: Joi.string().max(1000).optional().allow('').messages({
    'string.max': 'Additional notes cannot exceed 1000 characters'
  }),
  request_donation_pickup: Joi.boolean().optional().messages({
    'boolean.base': 'Request donation pickup must be a boolean value'
  }),
  request_demolition_addon: Joi.boolean().optional().messages({
    'boolean.base': 'Request demolition addon must be a boolean value'
  }),
  
  // Follow-up & Priority
  how_did_you_hear: Joi.string().max(255).optional().allow('').messages({
    'string.max': 'How did you hear cannot exceed 255 characters'
  }),
  request_priority: Joi.string().valid('standard', 'urgent', 'low').optional().messages({
    'any.only': 'Request priority must be one of: standard, urgent, low'
  }),
  
  // System Fields
  status: Joi.string().valid('pending', 'reviewed', 'quoted', 'accepted', 'declined', 'expired').optional().messages({
    'any.only': 'Status must be one of: pending, reviewed, quoted, accepted, declined, expired'
  }),
